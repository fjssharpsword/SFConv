...
def second():
    tb_log = SummaryWriter('/data/tmpexec/tb_log') #--port 10002
    ...
    label_img = torch.rand(100, 3, 10, 10)
    for i in range(100):
        label_img[i]*=i/100.0

    tb_log.add_embedding(torch.randn(100, 5), metadata=meta, label_img=label_img)
    tb_log.add_embedding(torch.randn(100, 5), label_img=label_img)
    tb_log.add_embedding(torch.randn(100, 5), metadata=meta)
